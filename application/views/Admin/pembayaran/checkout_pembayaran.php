      ...
      <div class="card">
        ...
        <div class="p-3">
          ...
          <div class="d-flex justify-content-between mb-2">
            ...
            <span><?= $data_pembayaran_permohonan->jabatan_kerja; ?>(
              <?= $data_pembayaran_permohonan->deskripsi_jabatan_kerja; ?> )</span>
          </div>
        </div>
        ...
        <div class="p-3">
          ...
          <div style="text-align:left; font-size:13px;">
            Catatan :<br />
            - Biaya pembayaran sertifikasi di LSP BPSDM Kementerian PU adalah gratis, klik tombol "Konfirmasi Bebas
            Biaya Sertifikasi" dibawah ini.<br />
            - Download format surat perjanjian sertifikasi dibawah ini untuk pengisian surat perjanjian
            sertifikasi.<br />
          ...
          if ($metode_pembayaran == 'Payment Gateway') {
              ...
              <table class="table table-bordered table-striped">
                ...
                <tbody>
                  <tr>
                    ...
                    <td><a href='<?= $get_status_pembayaran->pdf_url ?>' class='btn btn-primary' style='font-size:12px;'
            ...
            <?php if (empty($get_status_pembayaran->status_code) || $get_status_pembayaran->status_code == '201') { ?>
              <form action="<?= base_url('pembayaran/upload_bukti_pembayaran/') . base64_encode($id_izin); ?>" method="POST"
        ...
        <br />
        <hr />
        <form action="<?= base_url('pembayaran/upload_surat_perjanjian_sertifikasi/') . base64_encode($id_izin); ?>"
          method="POST" enctype="multipart/form-data">
          <input type="hidden" name="<?= $this->security->get_csrf_token_name(); ?>"
            value="<?= $this->security->get_csrf_hash(); ?>" />
          <h4>Upload File Surat Perjanjian Sertifikasi</h4>
          <?php if (empty($get_data_surat_perjanjian_sertifikat->file)) { ?>
            <a href="<?= base_url('assets/draf_dokumen/Surat Perjanjian Sertifikasi.docx') ?>" class="btn btn-warning"
              style="font-size:12px;">Download Draf Surat Perjanjian Sertifikasi Disini</a><br /><br />
          ...
          <?php } else { ?>
            <a href="<?= base_url('uploads/file_permohonan/surat_perjanjian_sertifikat/') . $get_data_surat_perjanjian_sertifikat->file; ?>"
              class="btn btn-success" target="_blank">Surat Perjanjian Sertifikat yang Sudah Diupload</a>
          <?php } ?>
        </form>
