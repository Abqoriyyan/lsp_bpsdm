...
class Mail extends MY_Controller
{
	...
	public function upload()
	{
		...
		if (empty($_POST['signed'])) {
			...
			echo "Error: Input data tidak ditemukan.";
			return;
		}
		...
		if (count($image_parts) !== 2) {
			http_response_code(400);
			echo "Error: Format data tidak valid.";
			return;
		}
		...
		if (count($image_type_aux) !== 2) {
			http_response_code(400);
			echo "Error: Format Mime Type tidak valid.";
			return;
		}

		$image_type = strtolower(preg_replace('/[^a-zA-Z0-9]/', '', $image_type_aux[1]));

		$allowed_extensions = ['png', 'jpg', 'jpeg', 'gif', 'webp'];

		if (!in_array($image_type, $allowed_extensions, true)) {
			http_response_code(403);
			echo "Error: Tipe file tidak diizinkan. Hanya menerima PNG, JPG, JPEG, GIF, atau WEBP.";
			return;
		}
		...
		if ($image_base64 === false) {
			http_response_code(400);
			echo "Error: Gagal melakukan decode data.";
			return;
		}

		$file_name = uniqid('sig_', true) . '.' . $image_type;
		...
		if (file_put_contents($file_path, $image_base64)) {
			...
			echo "Error: Gagal menyimpan file ke server.";
		}
	}
